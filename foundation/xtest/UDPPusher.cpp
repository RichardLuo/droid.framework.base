...
int UDPPusher::setRemoteAddress(const char *remote_host, int port) {
    struct sockaddr_in addr;
    struct hostent *ent = gethostbyname(remote_host);
    LOG_IF_RETURN(ent == NULL, "Failed to look up address of session host '%s'", remote_host);

    mRemoteAddr.sin_addr.s_addr = *(in_addr_t *)ent->h_addr;
    mRemoteAddr.sin_port = htons(port);
    return 0;
}
...
bool UDPPusher::onPush() {
    ...
    if (fread(&length, 1, sizeof(length), mFile) < sizeof(length)) {
        ddprint("# No more data to push.");
        return false;
    }

    LOGFL("length:%d", length);
...
#if CONF_NEW_UDP_PUSHER
            if (onPush() == false) {
                LOGFL("onPush failed!");
                this->looper()->stop();
                LOGFL("looper stopped!");
            }
#else            
